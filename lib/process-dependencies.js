 ...
 * the source list, and creates a list of dependencies for each.
 * When all of the dependencies for a source are resolved, it resolves
 * the source that declared the dependencies, which triggers a call to
 * that source function, passing in the resolved dependency data as 
 * parameters.
 *
 * Finally, when that function has resolved (either by returning the
 * source data, or calling the callback), that source is also resolved.
 */
'use strict';
var q = require('q'),
...
module.exports = function processDependencies(providers, dependencyList) {
  var dfd = q.defer(),
    loading = {},
    loaded = {},
    listLoaded;
  ...
  listLoaded = dependencyList.map(function (source) {
    ...
    if (dependencyNames.length) {
      ...
      q.all(dependencyPromises).then(function depFinished(depArgs) {
        ...
        // Load the source.
        response = loadSource.apply(providers, depArgs);
      ...
      response = loadSource.call(providers, src);
      handleResponse(loaded, name, response, dependencyDfd);
    }
